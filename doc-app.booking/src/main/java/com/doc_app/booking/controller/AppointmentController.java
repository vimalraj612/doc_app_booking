...
public class AppointmentController {
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "Created",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = AppointmentDTO.class)))
    @PostMapping
    public ResponseEntity<ApiResponse<AppointmentDTO>> createAppointment(@Valid @RequestBody CreateAppointmentRequest request) {
        ...
        return new ResponseEntity<>(ApiResponse.success("Appointment created successfully", appointmentDTO), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<ApiResponse<AppointmentDTO>> updateAppointmentStatus(@PathVariable Long id, @Valid @RequestBody AppointmentStatusUpdateRequest request) {
    ...
    public ResponseEntity<ApiResponse<PageResponse<AppointmentDTO>>> getAllAppointments(
        ...
        PageResponse<AppointmentDTO> response = appointmentService.getAllAppointments(pageNo, pageSize, sortBy, sortDir);
    ...
    public ResponseEntity<ApiResponse<List<AppointmentDTO>>> getAppointmentsByStatus(
            @Parameter(description = "Appointment status (e.g., SCHEDULED, CONFIRMED, CANCELLED)", required = true) 
    ...
    public ResponseEntity<ApiResponse<List<AppointmentDTO>>> getAppointmentsByDateRange(
            @Parameter(description = "Start date and time (yyyy-MM-dd'T'HH:mm:ss)", required = true)
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDate,
            @Parameter(description = "End date and time (yyyy-MM-dd'T'HH:mm:ss)", required = true)
    ...
    public ResponseEntity<ApiResponse<List<AppointmentDTO>>> getAppointmentsByDoctorAndDateRange(
        ...
        List<AppointmentDTO> appointments = appointmentService.getAppointmentsByDoctorAndDateRange(doctorId, start, end);
        return ResponseEntity.ok(ApiResponse.success(appointments));
    }
    ...
    public ResponseEntity<ApiResponse<List<AppointmentDTO>>> getAppointmentsByPatientAndDateRange(
            ...
            @Parameter(description = "End date and time (yyyy-MM-dd'T'HH:mm:ss)", required = true)
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime end) {
        List<AppointmentDTO> appointments = appointmentService.getAppointmentsByPatientAndDateRange(patientId, start, end);
        return ResponseEntity.ok(ApiResponse.success(appointments));
    }
    ...
    public ResponseEntity<ApiResponse<List<AppointmentDTO>>> getAppointmentsByHospitalAndDateRange(
            ...
            @Parameter(description = "End date and time (yyyy-MM-dd'T'HH:mm:ss)", required = true)
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime end) {
        List<AppointmentDTO> appointments = appointmentService.getAppointmentsByHospitalAndDateRange(hospitalId, start, end);
        return ResponseEntity.ok(ApiResponse.success(appointments));
    }
    ...
    public ResponseEntity<ApiResponse<Boolean>> isDoctorAvailable(
            @Parameter(description = "ID of the doctor", required = true) @PathVariable Long doctorId,
            @Parameter(description = "Date and time to check availability (yyyy-MM-dd'T'HH:mm:ss)", required = true)
